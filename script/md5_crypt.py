# script/md5_crypt.py
import crypt
# import colorama dihilangkan karena C sudah diimpor dari main.py

# --- Fungsi Cracker Utama ---
def crack_shadow_hash(shadow_entry, wordlist_file, C):
    """
    Memecahkan hash password MD5-Crypt dari /etc/shadow menggunakan wordlist.
    """
    ...
    try:
        ...
        if len(parts) < 2:
            print(C["ERROR"] + "Format entri /etc/shadow salah. (Harus ada delimiter ':').")
            return
        ...
        if full_hash.startswith('$1$'):
            ...
            if len(salt_parts) < 3:
                 print(C["ERROR"] + "Struktur hash MD5-Crypt tidak lengkap.")
                 return

            salt_crypt = f"${salt_parts[1]}${salt_parts[2]}" # Format: $1$t7y583it
        else:
            print(C["ERROR"] + "Format hash tidak didukung (Bukan MD5-Crypt $1$).")
            return
    except Exception as e:
        print(C["ERROR"] + f"Error parsing hash: {e}")
        return

    print(C["MENU"] + "\n--- PYTHON SHADOW CRACKER (MD5-Crypt) ---")
    print(C["MENU"] + f"[*] Target User: {username}")
    print(C["MENU"] + f"[*] Hash Type: MD5-Crypt ($1$)")
    print(C["MENU"] + f"[*] Salt: {salt_crypt}")
    print(C["MENU"] + f"[*] Memuat Wordlist dari: {wordlist_file}")
    # 2. Memulai Cracking
    try:
...
# --- TIDAK ADA KODE INPUT ATAU PEMANGGILAN FUNGSI DI SINI ---
