...
def execute(options):
    ...
    try:
        ...
        print(f"{C.MENU} Created Date:   {C.RESET}{get_clean_data(getattr(w, 'creation_date', None))}")
        print(f"{C.MENU} Expiry Date:    {C.RESET}{get_clean_data(getattr(w, 'expiration_date', None))}")
        print(f"{C.MENU} Organization:   {C.RESET}{getattr(w, 'private_registrant', 'N/A') if not getattr(w, 'org', None) else w.org}")
        
        # Email handling
        emails = getattr(w, 'emails', 'N/A')
        print(f"{C.MENU} Emails:         {C.RESET}{', '.join(emails) if isinstance(emails, list) else emails}")
        
        # Name Servers
        ns = getattr(w, 'name_servers', [])
        print(f"{C.MENU} Name Servers:   {C.RESET}{', '.join(list(ns)[:2]) if ns else 'N/A'}")
